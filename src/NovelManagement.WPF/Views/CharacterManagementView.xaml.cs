...
namespace NovelManagement.WPF.Views
{
    ...
    public partial class CharacterManagementView : UserControl, INavigationRefreshableView
    {
        ...
        private async void LoadCharactersAsync()
        {
            try
            {
                ...
                {
                    _logger?.LogWarning("没有当前项目，角色管理进入空状态");
                ...
                {
                    _logger?.LogWarning("角色服务不可用，角色管理进入空状态");
                    _allCharacters = new List<Character>();
                }
        ...
        /// 当项目切换时刷新角色列表
        /// </summary>
        /// <param name="projectId">新项目ID</param>
        /// <param name="projectName">新项目名称</param>
        /// <returns>表示异步操作的任务</returns>
        public Task RefreshOnProjectChangedAsync(Guid? projectId, string? projectName)
        {
            LoadCharactersAsync();
        ...
        {
            if (TotalCharactersTextBlock == null)
            {
                return;
            }

            var total = Math.Max(_allCharacters.Count, 1);
            var protagonistCount = _allCharacters.Count(c => c.Type == "主角");
            var supportingLeadCount = _allCharacters.Count(c => c.Type == "主配角");
            var antagonistCount = _allCharacters.Count(c => c.Type == "反派");

            TotalCharactersTextBlock.Text = _allCharacters.Count.ToString();
            MainCharactersTextBlock.Text = _allCharacters.Count(c => c.Importance >= 7).ToString();
            FactionCountTextBlock.Text = _allCharacters
                .Select(c => c.Faction?.Name)
                .Where(name => !string.IsNullOrWhiteSpace(name))
                .Distinct(StringComparer.OrdinalIgnoreCase)
                .Count()
                .ToString();
            RelationshipCountTextBlock.Text = _allCharacters.Sum(c => c.RelationshipsAsSource?.Count ?? 0).ToString();

            ProtagonistProgressBar.Value = protagonistCount;
            ProtagonistProgressBar.Maximum = total;
            ProtagonistCountTextBlock.Text = protagonistCount.ToString();

            SupportingLeadProgressBar.Value = supportingLeadCount;
            SupportingLeadProgressBar.Maximum = total;
            SupportingLeadCountTextBlock.Text = supportingLeadCount.ToString();

            AntagonistProgressBar.Value = antagonistCount;
            AntagonistProgressBar.Maximum = total;
            AntagonistCountTextBlock.Text = antagonistCount.ToString();
        }
        ...
        private void ImportCharacters_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                ProjectId = GetCurrentProjectId(),
                Source = "CharacterManagement.ImportCharacters",
                Payload = new ImportExportNavigationPayload
                {
                    Action = "CharactersImport"
                }
            });
        }
        ...
        private void ExportCharacters_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                ProjectId = GetCurrentProjectId(),
                Source = "CharacterManagement.ExportCharacters",
                Payload = new ImportExportNavigationPayload
                {
                    Action = "CharactersOnlyExport",
    ...
    public class CharacterGenerationDialog : Window
    {
        ...
        /// <summary>
        /// 初始化 CharacterGenerationDialog 的新实例
        /// </summary>
    ...
    public class CharacterOptimizationDialog : Window
    {
        ...
        /// <summary>
        /// 初始化 CharacterOptimizationDialog 的新实例
        /// </summary>
    ...
    {
        /// <summary>
        /// 初始化 RelationshipAnalysisDialog 的新实例
        /// </summary>
